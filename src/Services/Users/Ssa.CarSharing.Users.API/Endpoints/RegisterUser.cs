...
namespace Ssa.CarSharing.Users.API.Endpoints
{
    ...
    {
        internal record RegisterUserRequest(string FirstName, string LastName, string Email);
        public static void MapEndpoint(this IEndpointRouteBuilder app)
        {
            app.MapPost("/users/register", async (RegisterUserRequest request, ISender sender) =>
            {
                Result<Guid> result = await sender.Send(new RegisterUserCommand(request.FirstName, request.LastName, request.Email));
